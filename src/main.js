...
const domChangeListener = () => {
  ...
  listElement.click((event) => {
    const selectedAsset = currentAssetList.find((index) => index.uid === event.currentTarget.id);
...
$(document).ready(() => {
  ContentstackUIExtension.init().then((extension) => {
    ...
    storedData = Object.keys(storedData).length === 0 ? [] : storedData;
